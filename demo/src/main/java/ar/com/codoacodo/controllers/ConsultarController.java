	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(rs.next()) {//�hay datos?
				...
				Float cotización = rs.getFloat(3);
				Date fecha = rs.getDate(4);
				String imagen = rs.getString(5);
				String numero = rs.getString(6);
				String posición = rs.getString(7);
				
				//campos crear un objeto????
				Producto prodFromDb = new Producto(idProducto,nombre,cotización,fecha,imagen,numero,posición);
